...
SECTORS = {
    ...
    "金融政策": {
        "keywords": ["降准","降息","加息","增量资金","国家队","平准基金",
                     ...
                     "证监会","金融监管","资本市场改革","注册制",
                     "国九条","新国九条","活跃资本市场"],
        "stocks": ["中信证券","东方财富","工商银行","招商银行","建设银行",
                   "中国平安","中国人寿","同花顺","指南针","财富趋势"],
    },
}

SOURCE_WEIGHTS = {"东方财富":1.0,"财联社":1.2,"新浪财经":0.9}
POSITION_WEIGHTS = [1.0,0.95,0.9,0.85,0.8]

def classify_sectors(title, content=""):
    text = title + " " + content
    matched_sectors = {}
    for sector, data in SECTORS.items():
        ...
        for kw in data["keywords"]:
            if kw in text:
                ...
                score += 1
        if score > 0:
            matched_sectors[sector] = {"score": score, "keywords": matched_kws, "relevant_stocks": data["stocks"]}
    result = sorted(matched_sectors.items(), key=lambda x: x[1]["score"], reverse=True)
    return result[:3]

def map_stocks(title, sectors):
    stocks = {}
...
def score_news(news_item, position=0):
    from .config import BULLISH_KEYWORDS, BULLISH_SCORE, DEFAULT_SCORE, NOTIFY_THRESHOLD
    ...
    # 4. Specificity bonus (numbers, amounts, percentages)
    specificity_bonus = 0
    if re.search(r'\d+[%％]', title): specificity_bonus += 2
    if re.search(r'\d+亿', title): specificity_bonus += 3
    if re.search(r'\d+[万亿]', title): specificity_bonus += 4
    if re.search(r'[A-Z]+\d*', title): specificity_bonus += 1
    if re.search(r'[涨跌升降][停幅]', title): specificity_bonus += 2
    if re.search(r'[突破][性]', title): specificity_bonus += 2
    ...
    # 7. Signal level (more granular)
    if final_score >= 15:
        level = "重大利好"
        level_class = "level-high"
    elif final_score >= NOTIFY_THRESHOLD:
        level = "关注利好"
