...
from .config import TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, SERVERCHAN_KEY, BARK_KEY, BARK_SERVER, logger
...
def notify_all(title, message):
    ...
    full = f"\U0001F550 {now}\n{message}"
    r = {"tg": send_telegram(title, full), "sc": send_serverchan(title, full), "bark": send_bark(title, full)}
    if any(r.values()):
        logger.info(f"Push results: {r}")
    else:
        logger.info("No push channel configured - set TELEGRAM_BOT_TOKEN/TELEGRAM_CHAT_ID as GitHub Secrets")
    return r
